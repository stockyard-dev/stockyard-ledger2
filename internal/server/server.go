...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/entrys", s.list)
	s.mux.HandleFunc("POST /api/entrys", s.create)
	s.mux.HandleFunc("GET /api/entrys/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/entrys/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-ledger2", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"entrys": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Entry{} }
	wj(w, map[string]any{"entrys": items, "count": len(items)})
}
